#!/usr/bin/env python3
"""Run the PriVTE-Behavior v1 practical MVP."""
...
DEFAULT_CONFIG = Path("configs/algorithms/privte_behavior.v1.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the PriVTE-Behavior v1 practical MVP."
    )
    parser.add_argument(
        "--person-manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/quickstart/6_1data_privte_behavior_v1"),
    )
...
def main() -> int:
    ...
    if not args.config.exists():
        raise SystemExit(f"Behavior v1 config not found: {args.config}")
    ...
    try:
        result = write_evidence_run(
            ...
            extractor_name="privte_behavior_v1",
            extractor_config=extractor_config,
            evidence_stem="behavior_v1_mvp_evidence",
            report_stem="behavior_v1_mvp_report",
        )
    except RuntimeError as exc:
        raise SystemExit(
            f"{exc}\n"
            "For a plumbing check without behavior models, rerun with "
            "--allow-metadata-fallback. For real Behavior v1 output, provide "
            "the configured MediaPipe task models under models/mediapipe/."
        ) from exc
