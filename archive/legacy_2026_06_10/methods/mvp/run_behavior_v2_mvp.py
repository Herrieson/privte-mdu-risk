#!/usr/bin/env python3
"""Run the PriVTE-Behavior v2 practical MVP."""
...
DEFAULT_CONFIG = Path("configs/algorithms/privte_behavior.v2.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the PriVTE-Behavior v2 practical MVP."
    )
    parser.add_argument(
        "--person-manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/quickstart/6_1data_privte_behavior_v2"),
    )
...
def main() -> int:
    ...
    if not args.config.exists():
        raise SystemExit(f"Behavior v2 config not found: {args.config}")
    ...
    try:
        result = write_evidence_run(
            ...
            extractor_name="privte_behavior_v2",
            extractor_config=extractor_config,
            evidence_stem="behavior_v2_mvp_evidence",
            report_stem="behavior_v2_mvp_report",
        )
    except RuntimeError as exc:
        raise SystemExit(
            f"{exc}\n"
            "For a plumbing check without behavior models, rerun with "
            "--allow-metadata-fallback. For real Behavior v2 output, provide "
