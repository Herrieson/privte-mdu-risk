...
即：在本地把敏感视频转成结构化、质量感知、隐私过滤的文本证据，再让中心端 text-only LLM 做风险筛查判断。
...
4. **MDU-RiskBench 评测基准**
   ...
   构建 text-only LLM 评测基准，评估风险等级、置信度、证据使用、缺失信息、人类复核和隐私-效用权衡。
...
4. **LLM 对比**
   ...
   比较小型 LLM、强 LLM、rubric prompt、JSON constrained output。
...
使研究者可以在不接触原始视频的情况下评测 text-only LLM，
