#!/usr/bin/env python3
"""Run the PriVTE preprocessor v0 skeleton quickstart."""
...
DEFAULT_CONFIG = Path("configs/algorithms/privte_preprocessor.v0.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run PriVTE preprocessor v0 schema skeleton."
    )
    parser.add_argument(
        "--person-manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/quickstart/6_1data_privte_preprocessor_v0"),
    )
...
def main() -> int:
    ...
    result = write_evidence_run(
        ...
        extractor_name="privte_preprocessor_v0",
        extractor_config=read_config(args.config),
        evidence_stem="preprocessor_v0_evidence",
        report_stem="preprocessor_v0_report",
    )
